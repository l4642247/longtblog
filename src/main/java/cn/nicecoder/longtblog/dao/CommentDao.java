...
public interface CommentDao extends JpaRepository<Comment, Long>, JpaSpecificationExecutor {
    ...
    @Query(value="SELECT a.*,b.name as name,b.pic as pic FROM comment a LEFT JOIN user b on " +
            "a.userid = b.id WHERE discussid = ?1 AND a.type=?4 LIMIT ?2,?3 ;", nativeQuery = true)
    List<Comment> findByArtId(Long artId, int pageNo, int pageSize, String type);
    ...
    List<Comment> findByCommentId(Long CommentId, int pageNo, int pageSize, String type);
